...
def tutorial_dialog():
    ...
    st.write("O **🏠 Início** funciona como um painel único. No topo, **💰 Meu mês** reúne o alerta financeiro, os seis números principais e também **Próximo pagamento** e **Limite seguro/dia**. Esses indicadores não se repetem no final da página. Em **⚡ Ações rápidas**, registre receita, gasto ou conta sem procurar no menu. Logo abaixo ficam **Movimentações recentes**, o gráfico **Para onde foi meu dinheiro?** e **📅 Contas e compromissos**, onde continua disponível o botão **Paguei**.")
