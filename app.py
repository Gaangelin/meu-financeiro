...
page=st.sidebar.radio("Menu",["🏠 Início","🏦 Bancos","🤖 Assistente IA","🚦 Saúde Financeira","🔮 Previsão","➕ Registrar","🔁 Recorrentes","💳 Parcelamentos","🧾 Contas","💳 Cartões","📉 Dívidas","🎯 Metas","📊 Relatórios","⚙️ Configurações"])
...
if page=="🏠 Início":
    ...
    if st.session_state.get("abrir_posso_comprar",False):
        with st.expander("🛍️ Posso comprar? — Simulador",expanded=True):
            ...
            # Calcula os indicadores antes de usá-los no simulador.
            _pc_fs=financial_summary()
            _pc_saldo=float(_pc_fs.get("saldo_usar",0) or 0)
            _pc_limite=float(_pc_fs.get("limite_dia",0) or 0)
            saldo_depois=_pc_saldo-impacto
            ...
            if pc_valor>0:
                if saldo_depois<0:
                    st.error("🔴 Esta compra ultrapassa o seu saldo disponível considerando os compromissos atuais.")
                elif impacto>_pc_limite*7:
                    st.warning("🟡 A compra cabe no saldo atual, mas tem impacto relevante no seu orçamento. Confira seus próximos compromissos antes de confirmar.")
                else:
                    st.success("🟢 Pelo seu saldo disponível atual, esta compra cabe no orçamento. Continue preservando sua reserva e os próximos vencimentos.")
                if pc_forma=="Parcelado":
                    st.caption(f"{int(pc_parcelas)}x de {money(pc_parcela)} · total de {money(pc_valor)}. A análise usa a parcela mensal como novo compromisso.")
