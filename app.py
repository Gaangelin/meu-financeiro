...
if page=="🏠 Início":
    ...
    st.caption("Configure sua renda e quanto deseja preservar. Esses valores alimentam automaticamente os cálculos do Meu Financeiro.")
    if st.button("← Voltar ao Início",key="mr_back"):
        st.session_state["_page_destino"]="🏠 Início"
        st.rerun()
    st.divider()
    mr1,mr2=st.columns(2)
    renda1=mr1.number_input("1º recebimento / salário",min_value=0.0,value=float(cfg["rec1"]),step=100.0,format="%.2f",key="mr_rec1_page")
    renda2=mr2.number_input("2º recebimento / outra renda",min_value=0.0,value=float(cfg["rec2"]),step=100.0,format="%.2f",key="mr_rec2_page")
    mr3,mr4=st.columns(2)
    dia2=mr3.number_input("Dia do 2º recebimento",min_value=1,max_value=28,value=int(cfg["dia2"]),step=1,key="mr_dia2_page")
    pct_guardar=mr4.slider("Quanto quero guardar (%)",0,50,int(cfg["pct"]),key="mr_pct_page")
    reserva_min=st.number_input("Reserva mínima que quero preservar",min_value=0.0,value=float(cfg["reserva"]),step=100.0,format="%.2f",key="mr_reserva_page")
    ...
    if st.button("💾 Salvar minha renda",use_container_width=True,key="mr_salvar_page"):
        try:
            sb.table("config").update({
        ...
        if pc_forma=="À vista":
            if st.button("✅ Comprei — registrar gasto",use_container_width=True,key="pc_confirm_cash_page"):
                try:
                    ...
                    st.cache_data.clear()
                    st.session_state["_page_destino"]="🏠 Início"
                    st.session_state["_flash_inicio"]="✅ Compra registrada como gasto."
                    st.rerun()
                except Exception:
            ...
            if st.button("✅ Comprei — criar parcelamento",use_container_width=True,key="pc_confirm_installment_page"):
                try:
                    ...
                    st.cache_data.clear()
                    st.session_state["_page_destino"]="🏠 Início"
                    st.session_state["_flash_inicio"]="✅ Compra adicionada aos parcelamentos."
                    st.rerun()
                except Exception:
