...
if page=="🏠 Início":
    ...
    if _orcs:
        ...
        for _o in _orcs:
            ...
            _pct=min(_usado/_lim,1.0) if _lim>0 else 0
            bo1,bo2=st.columns([4,1])
            bo1.markdown(f"**{_cat}** · {money(_usado)} de {money(_lim)}")
            bo1.progress(_pct)
            if _lim>0 and _usado>_lim: bo2.error("Acima")
            elif _lim>0 and _usado>=_lim*.8: bo2.warning("Atenção")
            else: bo2.success("Ok")
    else:
...
Use Markdown simples e não coloque valores monetários entre crases ou blocos de código."""
    ...
    st.title("🎯 Metas")
    st.caption("Transforme objetivos em progresso visível.")
    ...
    _metas=myrows("metas")
    if not _metas: st.info("Você ainda não criou nenhuma meta.")
    for _m in _metas:
        _d=float(_m.get("desejado") or 0); _g=float(_m.get("guardado") or 0); _p=min(_g/_d,1.0) if _d>0 else 0
        st.markdown(f"#### 🎯 {_m.get('nome','Meta')}")
        st.progress(_p,text=f"{int(_p*100)}% concluído")
        a,b,c=st.columns(3);a.metric("Guardado",money(_g));b.metric("Objetivo",money(_d));c.metric("Falta",money(max(_d-_g,0)))
