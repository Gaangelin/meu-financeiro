...
if page=="🏠 Início":
    ...
    st.caption("Modo de teste gratuito. Nenhum banco real precisa ser conectado nesta etapa.")
    if not pluggy_configured():
        st.warning("Adicione PLUGGY_CLIENT_ID e PLUGGY_CLIENT_SECRET nos Secrets do Streamlit para liberar o teste.")
    else:
        if st.button("🏦 Conectar banco de teste",use_container_width=True,key="pluggy_open"):
            try:
                st.session_state["_pluggy_connect_token"]=pluggy_connect_token();st.session_state["_pluggy_widget_open"]=True
            except Exception as e: st.error(f"Não foi possível iniciar a conexão: {e}")
        if st.session_state.get("_pluggy_widget_open") and st.session_state.get("_pluggy_connect_token"):
            token=st.session_state["_pluggy_connect_token"]
            st.info("Abaixo, clique em **Abrir Pluggy Bank de teste**. Esse segundo clique acontece dentro do componente da Pluggy e evita o bloqueio que deixou a área vazia nas versões anteriores.")
            html=f"""
            <!doctype html>
            <html lang="pt-BR">
            <head>
              ...
              <style>
                body{{margin:0;background:#0e1117;color:#fff;font-family:Arial,sans-serif}}
                .box{{padding:18px;border:1px solid #334155;border-radius:12px;background:#151b26}}
                button{{width:100%;padding:15px;border:0;border-radius:9px;font-weight:700;cursor:pointer}}
                #status{{margin-top:12px;font-size:14px;color:#b8c5d6}}
              </style>
            </head>
            <body>
              <div class="box">
                <button id="open">🏦 Abrir Pluggy Bank de teste</button>
                <div id="status">Clique no botão acima para abrir a conexão segura.</div>
              </div>
              <script>
                const statusEl=document.getElementById('status');
                document.getElementById('open').addEventListener('click', function() {{
                  if (typeof PluggyConnect === 'undefined') {{
                    statusEl.textContent='❌ A biblioteca Pluggy Connect não carregou neste navegador.';
                    return;
                  }}
                  statusEl.textContent='Abrindo Pluggy Connect...';
                  try {{
                    const pluggyConnect = new PluggyConnect({{
                      connectToken: {json.dumps(token)},
                      includeSandbox: true,
                      language: 'pt',
                      onOpen: () => statusEl.textContent='✅ Pluggy Connect aberto.',
                      onSuccess: (data) => {{
                        const itemId = data && data.item ? data.item.id : (data && data.id ? data.id : '');
                        statusEl.textContent='✅ Banco de teste conectado.' + (itemId ? ' Item criado: '+itemId : '');
                      }},
                      onError: (error) => {{
                        statusEl.textContent='❌ Erro no Pluggy Connect. Tente novamente.';
                        console.error(error);
                      }},
                      onClose: () => {{
                        if (!statusEl.textContent.includes('conectado')) statusEl.textContent='Janela da Pluggy fechada.';
                      }}
                    }});
                    pluggyConnect.init();
                  }} catch(e) {{
                    statusEl.textContent='❌ Falha ao iniciar o widget: '+e.message;
                    console.error(e);
                  }}
                }});
              </script>
            ...
            if st.button("✖ Fechar conexão",use_container_width=True,key="pluggy_close"):
                st.session_state["_pluggy_widget_open"]=False;st.rerun()
        st.markdown("#### 🔄 Sincronização")
        st.write("Depois da conexão, traga contas, saldos e movimentações para o Meu Financeiro.")
        if st.button("🔄 Sincronizar dados da Pluggy",use_container_width=True,key="pluggy_sync"):
            with st.spinner("Buscando dados do banco de teste..."):
                try:
                    ni,na,nt=sync_pluggy_data()
                    st.success(f"Sincronização concluída: {ni} conexão(ões), {na} conta(s) e {nt} nova(s) movimentação(ões).")
                except Exception as e:
                    msg=str(e)
                    if "LIST_ITEMS_FEATURE_NOT_ENABLED" in msg or "HTTP 403" in msg:
                        st.warning("Sua conta Pluggy ainda não liberou a listagem de conexões (/v2/items). A integração está pronta, mas será necessário habilitar **List Items** no time Pluggy para a sincronização automática.")
                    else: st.error(f"Não foi possível sincronizar: {msg}")
        st.caption("🔒 Client Secret e API Key ficam somente no servidor; o navegador recebe apenas um Connect Token temporário.")
