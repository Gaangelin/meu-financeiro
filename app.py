...
if page=="🏠 Início":
    ...
    st.caption("Modo de desenvolvimento. Use somente o Pluggy Bank de teste nesta etapa.")
    st.info("🔒 O Client Secret permanece no servidor. O componente recebe somente um Connect Token temporário e limitado.")
    ...
    if not pluggy_configured():
        ...
        if st.button("🔐 Gerar conexão segura",use_container_width=True,key="pluggy_prepare_v63"):
            try:
                st.session_state["_pluggy_v63_token"]=pluggy_connect_token()
                st.success("Conexão segura preparada. Use o botão abaixo.")
            except Exception as e:
                st.error(f"Não foi possível preparar a conexão: {e}")

        token=st.session_state.get("_pluggy_v63_token")
        if token:
            result=pluggy_connect_component(
                data={"connectToken":token},
                default={"connected":None,"connect_error":None},
                key="pluggy_connect_v63",
            ...
            if item_id:
                ...
                st.success("✅ Banco de teste conectado. O Item ID foi recebido pelo Meu Financeiro.")

        saved_item=st.session_state.get("_pluggy_item_id")
        if saved_item:
            st.markdown("#### 🔄 Importar dados do banco de teste")
            st.caption("A sincronização usa diretamente o Item recém-conectado; não depende da listagem geral de Items.")
            if st.button("🔄 Sincronizar conta, saldo e movimentações",use_container_width=True,key="pluggy_sync_v63"):
                with st.spinner("Sincronizando dados da Pluggy..."):
                    try:
                        ...
                        st.session_state["_pluggy_v63_token"]=None
                    except Exception as e:
                        st.error(f"Não foi possível sincronizar: {e}")

        with st.expander("🧪 Dados do Pluggy Bank para o teste"):
            st.code("Usuário: user-ok\nSenha: password-ok\nMFA (se pedir): 123456",language=None)
            st.caption("São credenciais fictícias do Sandbox oficial da Pluggy. Não use dados de banco real.")
