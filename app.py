...
def tutorial_dialog():
    ...
    st.write("Na tela **🏠 Início**, clique em **⚙️ Minha renda** e informe seus recebimentos, quanto deseja guardar e a reserva mínima que quer preservar. Esses valores alimentam automaticamente os cálculos do Meu Financeiro.")
    ...
    st.write("Na tela **🏠 Início**, os atalhos **Recebi dinheiro**, **Gastei dinheiro** e **Tenho uma conta** ajudam usuários iniciantes a encontrar rapidamente a função certa. Em **Posso comprar?**, informe o valor e escolha à vista ou parcelado para ver o impacto no saldo antes da compra. Se decidir comprar, o próprio simulador pode registrar o gasto ou criar o parcelamento. Em compras parceladas, informe também a data da primeira parcela; o sistema considera o compromisso dos próximos meses.")
...
if page_destino:
    ...
    page=st.sidebar.radio("Menu",menu_visivel)
...
if page=="🏠 Início":
    ...
    if acao_simples=="recebi":
        ...
        st.rerun()
    elif acao_simples=="comprar":
        st.session_state["abrir_posso_comprar"]=True

    if st.session_state.get("abrir_minha_renda",False):
        with st.expander("⚙️ Minha renda — Configuração rápida",expanded=True):
            st.caption("Informe quanto entra normalmente no mês. Esses são os mesmos valores usados nos cálculos do Meu Financeiro.")
            mr1,mr2=st.columns(2)
            renda1=mr1.number_input("1º recebimento / salário",min_value=0.0,value=float(cfg["rec1"]),step=100.0,format="%.2f",key="mr_rec1")
            renda2=mr2.number_input("2º recebimento / outra renda",min_value=0.0,value=float(cfg["rec2"]),step=100.0,format="%.2f",key="mr_rec2")
            mr3,mr4=st.columns(2)
            dia2=mr3.number_input("Dia do 2º recebimento",min_value=1,max_value=28,value=int(cfg["dia2"]),step=1,key="mr_dia2")
            pct_guardar=mr4.slider("Quanto quero guardar (%)",0,50,int(cfg["pct"]),key="mr_pct")
            reserva_min=st.number_input("Reserva mínima que quero preservar",min_value=0.0,value=float(cfg["reserva"]),step=100.0,format="%.2f",key="mr_reserva")
            renda_total=renda1+renda2
            st.info(f"Renda mensal configurada: **{money(renda_total)}** · Meta para guardar: **{money(renda_total*pct_guardar/100)}**")
            q1,q2=st.columns(2)
            if q1.button("💾 Salvar minha renda",use_container_width=True,key="mr_salvar"):
                try:
                    sb.table("config").update({
                    ...
                    st.cache_data.clear()
                    st.session_state["abrir_minha_renda"]=False
                    st.success("Renda atualizada.")
                    st.rerun()
                except Exception:
                    st.error("Não foi possível salvar sua renda.")
            if q2.button("Fechar",use_container_width=True,key="mr_fechar"):
                st.session_state["abrir_minha_renda"]=False
                st.rerun()

    if st.session_state.get("abrir_posso_comprar",False):
        with st.expander("🛍️ Posso comprar? — Simulador",expanded=True):
            st.caption("Simule antes de comprar. Nada é lançado no seu financeiro até você confirmar.")
            pc1,pc2=st.columns(2)
            pc_nome=pc1.text_input("O que você quer comprar?",placeholder="Ex.: Celular",key="pc_nome")
            pc_valor=pc2.number_input("Valor total da compra",min_value=0.0,step=50.0,format="%.2f",key="pc_valor")
            pc3,pc4=st.columns(2)
            pc_forma=pc3.radio("Como pretende pagar?",["À vista","Parcelado"],horizontal=True,key="pc_forma")
            pc_parcelas=1
            pc_primeiro_venc=None
            if pc_forma=="Parcelado":
                pc_parcelas=pc4.number_input("Quantidade de parcelas",min_value=2,max_value=60,value=10,step=1,key="pc_parcelas")
                pc_primeiro_venc=st.date_input("Quando vence a primeira parcela?",value=date.today(),key="pc_primeiro_venc")
            else:
                pc4.write("")
            pc_parcela=(pc_valor/pc_parcelas) if pc_parcelas else pc_valor
            impacto=pc_valor if pc_forma=="À vista" else pc_parcela

            saldo_depois=saldo-impacto
            ...
            if pc_valor>0:
                renda_base=float(cfg.get("renda") or 0)
                reserva_base=float(cfg.get("reserva") or 0)
                margem_mensal=max(renda_base-reserva_base-contas_pend-rec_pend-parc_pend-gastos,0)
                meses_comprometidos=int(pc_parcelas) if pc_forma=="Parcelado" else 1
                comprometimento_total=impacto*meses_comprometidos
                ...
                elif impacto>diario*7 or (pc_forma=="Parcelado" and margem_mensal>0 and pc_parcela>margem_mensal*0.30):
                ...
                st.markdown("#### Adicionar ao Meu Financeiro")
                st.caption("Só confirme abaixo se a compra realmente foi feita.")
                if pc_forma=="À vista":
                    if st.button("✅ Comprei — registrar gasto",use_container_width=True,key="pc_confirm_cash"):
                        try:
                            sb.table("mov").insert({
                                "user_id":st.session_state.uid,
                                "tipo":"Saída",
                                "categoria":"Compras",
                                "descricao":pc_nome.strip() or "Compra simulada",
                                "valor":float(pc_valor),
                                "data":date.today().isoformat()
                            }).execute()
                            st.cache_data.clear()
                            st.success("Compra registrada como gasto.")
                            st.session_state["abrir_posso_comprar"]=False
                            st.rerun()
                        except Exception:
                            st.error("Não foi possível registrar automaticamente. Use ➕ Registrar para lançar esta compra.")
                else:
                    if st.button("✅ Comprei — criar parcelamento",use_container_width=True,key="pc_confirm_installment"):
                        try:
                            sb.table("parcelamentos").insert({
                                "user_id":st.session_state.uid,
                                "nome":pc_nome.strip() or "Compra parcelada",
                                "categoria":"Compras",
                                "valor_parcela":float(pc_parcela),
                                "total_parcelas":int(pc_parcelas),
                                "parcelas_pagas":0,
                                "dia_vencimento":int(pc_primeiro_venc.day),
                                "forma":"Crédito",
                                "ativo":True
                            }).execute()
                            st.cache_data.clear()
                            st.success("Compra adicionada aos parcelamentos.")
                            st.session_state["abrir_posso_comprar"]=False
                            st.rerun()
                        except Exception:
                            st.error("Não foi possível criar o parcelamento automaticamente. Use 💳 Parcelamentos para cadastrar esta compra.")

            if st.button("Fechar simulador",key="pc_close"):
                st.session_state["abrir_posso_comprar"]=False
                st.rerun()

    st.divider()

    with st.expander("📂 Meus compromissos"):
        st.caption("Tudo continua salvo. O menu ficou menor para facilitar o uso.")
        h1,h2,h3=st.columns(3)
        h1.markdown("🧾 **Contas**  \nVencimentos e contas cadastradas.")
        h2.markdown("🔁 **Recorrentes**  \nGastos que se repetem.")
        h3.markdown("💳 **Parcelamentos e cartões**  \nCompras parceladas e faturas.")
        st.info("Nesta etapa, nenhuma função foi apagada. Estamos apenas simplificando a navegação antes de trazer essas ações para o Início.")
