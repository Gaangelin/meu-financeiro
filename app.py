...
def login():
    """
    V17 — Tela de entrada com vídeo real em loop.
    Coloque um arquivo chamado login_bg.mp4 na mesma pasta do app.py.
    O vídeo fica parado como câmera; o movimento deve estar DENTRO do vídeo.
    """
    ...
    st.markdown(f"""
    <style>
    /* LOGIN V17 — vídeo em tela cheia, câmera fixa */
    ...
    .stApp {{
        background:
          radial-gradient(circle at 22% 40%, rgba(0,119,255,.22), transparent 35%),
          linear-gradient(120deg,#020813 0%,#06172b 52%,#020711 100%) !important;
        overflow-x:hidden;
    }}
    .block-container {{
    ...
    .stApp::before {{
        ...
        pointer-events:none;
        background:
          linear-gradient(90deg,rgba(1,7,16,.20) 0%,rgba(1,7,16,.08) 48%,rgba(1,7,16,.48) 100%),
    ...
    [data-testid="stTabs"] {{
        background:rgba(4,17,34,.80);
        border:1px solid rgba(74,166,255,.30);
        border-radius:24px;
    ...
    [data-testid="stTabs"] [data-baseweb="input"] > div {{
        background:rgba(2,12,25,.76)!important;
        border-color:rgba(126,190,255,.28)!important;
    }}
    [data-testid="stTabs"] input {{
        color:white!important;
        -webkit-text-fill-color:white!important;
    }}
    [data-testid="stTabs"] div[data-testid="stFormSubmitButton"] button {{
        background:linear-gradient(90deg,#087cf0,#1ab7ff)!important;
        color:white!important;
    ...
    [data-testid="stTabs"] .stExpander {{
        background:rgba(2,12,25,.48)!important;
        border-radius:12px;
    }}
    ...
    st.markdown(
        '<div class="mf-login-sub">Seu dinheiro sob controle. Planeje, acompanhe e conquiste seus objetivos.</div>',
        unsafe_allow_html=True
    )
