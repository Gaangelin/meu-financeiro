...
def tutorial_dialog():
    ...
    st.write("No topo do **🏠 Início**, **👀 Sua situação agora** mostra em linguagem simples quanto você pode usar, quanto ainda falta pagar e quantos compromissos estão atrasados. Em **📂 Meus compromissos**, os itens ficam em ordem de vencimento com sinais de prioridade e o botão **Paguei**.")
...
if page=="🏠 Início":
    ...
    _fs_inicio=financial_summary()
    _saldo_inicio=float(_fs_inicio.get("saldo") or 0)
    _pend_inicio=float(_fs_inicio.get("contas_pend") or 0)+float(_fs_inicio.get("recorrentes_pend") or 0)+float(_fs_inicio.get("parcelas_pend") or 0)
    _atrasadas_inicio=0
    ...
    if acao_simples=="recebi":
        st.session_state["_page_destino"]="💰 Recebi dinheiro"
        st.rerun()
    elif acao_simples=="gastei":
        st.session_state["_page_destino"]="🛒 Gastei dinheiro"
        st.rerun()
    elif acao_simples=="conta":
        st.session_state["_page_destino"]="🧾 Tenho uma conta"
        st.rerun()
