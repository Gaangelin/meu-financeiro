...
def tutorial_dialog():
    ...
    st.markdown("### 8️⃣ Conheça a Central de Bancos")
    st.write("Em **🏦 Bancos**, você encontra a área preparada para futuras conexões via Open Finance. Nesta versão, a conexão real ainda não está ativada e o app não solicita senha bancária, CVV ou número completo do cartão.")
...
if page=="🏠 Início":
    ...
    st.caption("Uma área preparada para reunir suas contas bancárias e, futuramente, conectar dados via Open Finance.")

    st.info("🔒 O Meu Financeiro nunca deve pedir sua senha bancária, CVV ou número completo do cartão. A futura conexão será feita por autorização no ambiente seguro do banco/provedor Open Finance.")

    c1,c2,c3=st.columns(3)
    c1.metric("Bancos conectados","0")
    c2.metric("Saldo sincronizado",money(0))
    c3.metric("Última atualização","—")

    st.subheader("Conectar uma instituição")
    st.write("Quando a integração Open Finance estiver habilitada, este botão abrirá uma autorização segura para você escolher o banco e permitir somente os dados necessários.")
    st.button("🔗 Conectar meu banco",use_container_width=True,disabled=True)
    st.caption("Em breve — nenhuma credencial bancária é solicitada nesta versão.")

    st.divider()
    st.subheader("Como vai funcionar")
    a,b,c=st.columns(3)
    with a:
        st.markdown("#### 1️⃣ Escolher")
        st.write("Você escolhe a instituição que deseja conectar.")
    with b:
        st.markdown("#### 2️⃣ Autorizar")
        st.write("A autorização acontece fora do Meu Financeiro, no fluxo seguro do banco/provedor.")
    with c:
        st.markdown("#### 3️⃣ Sincronizar")
        st.write("Depois da autorização, o app poderá organizar saldos e movimentações permitidas.")

    st.divider()
    st.subheader("Suas instituições")
    st.info("Nenhum banco conectado ainda. Seus lançamentos manuais continuam funcionando normalmente.")
