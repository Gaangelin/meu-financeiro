...
if page=="🏠 Início":
    ...
    if not pluggy_configured():
        ...
        if st.session_state.get("_pluggy_widget_open") and st.session_state.get("_pluggy_connect_token"):
            ...
            st.info("Escolha **Pluggy Bank** para o teste. A janela abaixo é fornecida pela Pluggy.")
            html=f"""
            <div id="pluggy-connect" style="min-height:650px;font-family:Arial;color:#ddd;padding:12px">Carregando conexão segura da Pluggy...</div>
            <script src="https://cdn.pluggy.ai/pluggy-connect/v2.7.0/pluggy-connect.js"></script>
            <script>
              if (typeof PluggyConnect === 'undefined') {{
                document.getElementById('pluggy-connect').innerHTML =
                  '<div style="padding:20px;border:1px solid #b94a48;border-radius:10px;color:#ffd6d6">Não foi possível carregar o Pluggy Connect. Atualize a página e tente novamente.</div>';
              }} else {{
              const connect = new PluggyConnect({{
                ...
                language: 'pt',
                onSuccess: (data) => {{
                  document.getElementById('pluggy-connect').innerHTML='<div style="font-family:Arial;padding:24px;border-radius:12px;background:#123b2b;color:#d8ffe9"><b>✅ Banco de teste conectado.</b><br><br>Agora clique em “Sincronizar dados” no Meu Financeiro.</div>';
                }},
