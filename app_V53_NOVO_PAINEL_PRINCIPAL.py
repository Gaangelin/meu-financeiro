...
def tutorial_dialog():
    ...
    st.write("O **🏠 Início** agora funciona como um painel único. No topo, **👀 Sua situação agora** explica como está o mês. Em **📊 Visão geral**, você vê disponível, entradas, gastos, contas a pagar, valor para guardar e atrasos. Em **⚡ Ações rápidas**, registre receita, gasto ou conta sem procurar no menu. Logo abaixo ficam **Movimentações recentes**, o gráfico **Para onde foi meu dinheiro?** e **📅 Contas e compromissos**, onde continua disponível o botão **Paguei**.")
...
if page=="🏠 Início":
    ...
    st.markdown("### 👀 Sua situação agora")
    if _saldo_bruto_inicio>=0 and _atrasadas_inicio==0:
        st.success("✅ Seu dinheiro está dentro do planejado e não há compromissos atrasados.")
    elif _saldo_bruto_inicio>=0:
        st.warning(f"⚠️ Seu saldo ainda está positivo, mas você tem {_atrasadas_inicio} compromisso(s) atrasado(s).")
    else:
        st.error("🔴 Seus compromissos ultrapassaram o valor disponível planejado para este mês.")
    rs1,rs2,rs3=st.columns(3)
    rs1.metric("💵 Você pode usar",money(_saldo_inicio))
    rs2.metric("🧾 Ainda falta pagar",money(_pend_inicio))
    rs3.metric("⏰ Atrasados",str(_atrasadas_inicio))
    st.caption("Esse resumo considera sua renda, gastos realizados, valor para guardar e compromissos pendentes do mês.")
    st.divider()
    ...
    if qa1.button("➕ Receber",use_container_width=True,key="dash_receber"):
        st.session_state["_page_destino"]="💰 Recebi dinheiro";st.rerun()
    if qa2.button("➖ Gastar",use_container_width=True,key="dash_gastar"):
        st.session_state["_page_destino"]="🛒 Gastei dinheiro";st.rerun()
    if qa3.button("🧾 Nova conta",use_container_width=True,key="dash_conta"):
        st.session_state["_page_destino"]="🧾 Tenho uma conta";st.rerun()
    ...
    with painel_esq:
        ...
        if len(mm):
            _recent=mm.sort_values("data",ascending=False).head(6)
